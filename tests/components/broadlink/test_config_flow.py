...
BROADCAST_ADDRS = "homeassistant.components.broadlink.config_flow.get_broadcast_addrs"
DISCOVERY = "homeassistant.components.broadlink.config_flow.blk.discover"
FACTORY = "homeassistant.components.broadlink.config_flow.blk.gendevice"
HELLO = "homeassistant.components.broadlink.config_flow.blk.hello"
...
async def test_flow_user_already_in_progress(hass):
    ...
    with patch(HELLO, return_value=device.get_mock_api()):
        result = await hass.config_entries.flow.async_configure(
            result["flow_id"],
            {"host": device.host, "timeout": device.timeout},
        )
...
async def test_flow_user_mac_already_configured(hass):
    ...
    with patch(HELLO, return_value=mock_api):
        result = await hass.config_entries.flow.async_configure(
            result["flow_id"],
            {"host": device.host, "timeout": device.timeout},
        )
...
async def test_flow_user_invalid_ip_address(hass):
    ...
    with patch(HELLO, side_effect=OSError(errno.EINVAL, None)):
        result = await hass.config_entries.flow.async_configure(
            result["flow_id"],
            {"host": "0.0.0.1"},
        )
...
async def test_flow_user_invalid_hostname(hass):
    ...
    with patch(HELLO, side_effect=OSError(socket.EAI_NONAME, None)):
        result = await hass.config_entries.flow.async_configure(
            result["flow_id"],
...
async def test_flow_user_device_not_found(hass):
    ...
    with patch(HELLO, side_effect=blke.NetworkTimeoutError()):
        result = await hass.config_entries.flow.async_configure(
            result["flow_id"],
            {"host": device.host},
        )
...
async def test_flow_user_network_unreachable(hass):
    ...
    with patch(HELLO, side_effect=OSError(errno.ENETUNREACH, None)):
        result = await hass.config_entries.flow.async_configure(
            result["flow_id"],
            {"host": "192.168.1.32"},
        )
...
async def test_flow_user_os_error(hass):
    ...
    with patch(HELLO, side_effect=OSError()):
        result = await hass.config_entries.flow.async_configure(
            result["flow_id"],
            {"host": "192.168.1.32"},
        )
...
async def test_flow_discover_one_device_one_netif(hass):
    """Test a config flow with a single network and one device discovered."""
    device = get_device("Living Room")
    mock_api = device.get_mock_api()
    ...
    with patch(BROADCAST_ADDRS, return_value=["192.168.0.255"]), patch(
        DISCOVERY, return_value=[mock_api]
...
async def test_flow_discover_many_devices_one_netif(hass):
    """Test a config flow with a single network and many devices discovered."""
    device = get_device("Living Room")
    mock_api = device.get_mock_api()
...
async def test_flow_discover_many_devices_mult_netifs(hass):
    """Test a config flow with multiple networks and many devices discovered."""
    device = get_device("Living Room")
    mock_api = device.get_mock_api()
...
async def test_flow_discover_network_unreachable(hass):
    """Test we handle a network unreachable during a device discovery."""
    result = await hass.config_entries.flow.async_init(
        DOMAIN, context={"source": config_entries.SOURCE_USER}
    )

    with patch(BROADCAST_ADDRS, return_value=["192.168.0.255"]), patch(
        DISCOVERY, side_effect=OSError(errno.ENETUNREACH, None)
    ):
        result = await hass.config_entries.flow.async_configure(result["flow_id"], {})
...
async def test_flow_discover_os_error(hass):
    """Test we handle an OSError during a device discovery."""
    result = await hass.config_entries.flow.async_init(
        DOMAIN, context={"source": config_entries.SOURCE_USER}
    )

    with patch(BROADCAST_ADDRS, return_value=["192.168.0.255"]), patch(
        DISCOVERY, side_effect=OSError()
    ):
        result = await hass.config_entries.flow.async_configure(result["flow_id"], {})
...
async def test_flow_integration_discovery_works(hass):
    ...
    with patch(FACTORY, return_value=mock_api) as mock_factory:
        result = await hass.config_entries.flow.async_init(
            DOMAIN,
...
async def test_flow_integration_discovery_already_in_progress(hass):
    ...
    with patch(FACTORY, return_value=device.get_mock_api()):
        result = await hass.config_entries.flow.async_init(
            DOMAIN,
            context={"source": config_entries.SOURCE_INTEGRATION_DISCOVERY},
            data=data,
...
async def test_flow_integration_discovery_host_already_configured(hass):
    ...
    with patch(FACTORY, return_value=mock_api):
        result = await hass.config_entries.flow.async_init(
            DOMAIN,
            context={"source": config_entries.SOURCE_INTEGRATION_DISCOVERY},
            data=device.get_discovery_info(),
