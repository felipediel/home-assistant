...
DEVICE_DISCOVERY = "homeassistant.components.broadlink.updater.blk.discover"
...
async def test_setup_new_devices_discovered(hass):
    ...
    with patch(DEVICE_DISCOVERY, return_value=mock_apis) as mock_discover, patch.object(
...
async def test_setup_no_devices_discovered(hass):
    ...
    with patch(DEVICE_DISCOVERY, return_value=[]) as mock_discover, patch.object(
        hass.config_entries.flow, "async_init"
    ) as mock_init:
        assert await async_setup_component(hass, broadlink.DOMAIN, {}) is True
...
async def test_setup_discover_already_known_host(hass):
    ...
    with patch(
        DEVICE_DISCOVERY, return_value=[mock_api]
...
async def test_setup_discover_update_ip_address(hass):
    ...
    with patch(DEVICE_DISCOVERY, return_value=[mock_api]) as mock_discover, patch(
