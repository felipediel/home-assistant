"""Heartbeat for Broadlink devices."""
...
class BroadlinkHeartbeat:
    """Manages heartbeats to feed watchdog timers locally."""
    ...
    def __init__(self, hass):
        ...
        self.coordinator = DataUpdateCoordinator(
            ...
            _LOGGER,
            name="heartbeat",
            update_method=self.async_heartbeat,
    ...
    async def async_heartbeat(self):
        """Send heartbeat messages to feed watchdog timers."""
        hass = self.hass
        broadcast_addrs = hass.data[DOMAIN].config["broadcast_addrs"]
        current_entries = hass.config_entries.async_entries(DOMAIN)

        await hass.async_add_executor_job(
            self.heartbeat, broadcast_addrs, current_entries
        )

    def heartbeat(self, broadcast_addrs, current_entries):
        """Send heartbeat messages to feed watchdog timers."""
        ...
        for addr in chain(broadcast_addrs, uncovered_hosts):
            try:
                ...
                _LOGGER.debug("Failed to send heartbeat message to %s: %s", addr, err)
